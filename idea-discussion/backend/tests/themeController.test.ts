...
describe("getAllThemes コントローラー", () => {
  ...
  describe("includeInactive パラメータによるフィルタリング", () => {
    ...
    test("非管理者が includeInactive=true を指定しても、公開中・終了済みテーマのみのフィルタを使用すること", async () => {
      mockThemeFindSorted([]);
    ...
    test("includeInactive パラメータなしの場合、公開中・終了済みテーマのフィルタを使用すること", async () => {
    ...
    test("includeInactive=false の場合、公開中・終了済みテーマのみのフィルタを使用すること", async () => {
      mockThemeFindSorted([]);
    ...
    test("一般ユーザーは archived テーマが含まれないフィルタを使用すること", async () => {
      mockThemeFindSorted([]);

      const { req, res } = createMockReqRes();
      await getAllThemes(req as unknown as Request, res as unknown as Response);

      const findCall = (Theme.find as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
      expect(findCall).not.toEqual({ status: "archived" });
      expect(findCall).toEqual({ status: { $in: ["active", "closed"] } });
    });
...
describe("updateTheme コントローラー - ステータスベースプロンプトロック制御", () => {
  ...
  describe("ステータス遷移バリデーション", () => {
    ...
    test("closed → archived の遷移は許可されること", async () => {
      const mockTheme = createMockThemeDoc({ status: "closed" });
      (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(mockTheme);
      (Theme.findByIdAndUpdate as ReturnType<typeof vi.fn>).mockResolvedValue({
        ...mockTheme,
      ...
      const { req, res } = createUpdateMockReqRes({ status: "archived" });
      await updateTheme(req as unknown as Request, res as unknown as Response);

      expect(res.status).toHaveBeenCalledWith(200);
    });

    test("archived → closed の遷移は許可されること（再公開のため）", async () => {
      const mockTheme = createMockThemeDoc({ status: "archived" });
      (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(mockTheme);
      (Theme.findByIdAndUpdate as ReturnType<typeof vi.fn>).mockResolvedValue({
        ...mockTheme,
        status: "closed",
    ...
    test("archived → active の遷移は拒否されること", async () => {
      (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(
        createMockThemeDoc({ status: "archived" })
      );
    ...
    test("archived → draft の遷移は拒否されること", async () => {
      (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(
        createMockThemeDoc({ status: "archived" })
      );
...
describe("getThemeById コントローラー - アクセス制御", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  /**
   * getThemeById 用のリクエスト・レスポンスモックを生成するヘルパー関数
   */
  const createThemeByIdReqRes = (
    themeId = VALID_THEME_ID,
    user: { role: string } | undefined = undefined
  ) => ({
    req: { params: { themeId }, user },
    res: {
      status: vi.fn().mockReturnThis(),
      json: vi.fn().mockReturnThis(),
    },
  ...
  test("archived テーマに非管理者がアクセスすると 404 を返すこと", async () => {
    (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(
      createMockThemeDoc({ status: "archived" })
    );

    const { req, res } = createThemeByIdReqRes(VALID_THEME_ID, undefined);
    await getThemeById(req as unknown as Request, res as unknown as Response);

    expect(res.status).toHaveBeenCalledWith(404);
  });

  test("draft テーマに非管理者がアクセスすると 404 を返すこと", async () => {
    (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(
      createMockThemeDoc({ status: "draft" })
    );
  ...
  test("archived テーマに管理者がアクセスすると 200 を返すこと", async () => {
    (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(
      createMockThemeDoc({ status: "archived" })
    );

    const { req, res } = createThemeByIdReqRes(VALID_THEME_ID, {
      role: "admin",
    });
    await getThemeById(req as unknown as Request, res as unknown as Response);
  ...
  test("draft テーマに管理者がアクセスすると 200 を返すこと", async () => {
    (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(
      createMockThemeDoc({ status: "draft" })
    );
  ...
  test("active テーマには非管理者もアクセスできること", async () => {
    (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(
      createMockThemeDoc({ status: "active" })
    );
  ...
  test("closed テーマには非管理者もアクセスできること", async () => {
    (Theme.findById as ReturnType<typeof vi.fn>).mockResolvedValue(
      createMockThemeDoc({ status: "closed" })
    );
...
describe("getThemeDetail コントローラー - アクセス制御", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  const createDetailWithUserReqRes = (
    themeId = VALID_THEME_ID,
    user: { role: string } | undefined = undefined
  ) => ({
    req: { params: { themeId }, user },
  ...
  test("archived テーマに非管理者がアクセスすると 404 を返すこと", async () => {
    ...
    const { req, res } = createDetailWithUserReqRes(VALID_THEME_ID, undefined);
    await getThemeDetail(req as unknown as Request, res as unknown as Response);
  ...
  test("archived テーマに管理者がアクセスすると 200 を返すこと", async () => {
    ...
    const { req, res } = createDetailWithUserReqRes(VALID_THEME_ID, {
      role: "admin",
    });
    await getThemeDetail(req as unknown as Request, res as unknown as Response);
